...
import { z } from 'zod';
import { dealImportRowSchema } from '@/src/validation';
...
export async function POST(request: NextRequest) {
  ...
  const valid: (z.infer<typeof dealImportRowSchema> & { status: 'Open' })[] = [];
  const errors: { row: number; message: string }[] = [];

  parsedBody.data.rows.forEach((row, index) => {
    const parsed = dealImportRowSchema.safeParse(row);
    if (parsed.success) {
      valid.push({ ...parsed.data, status: 'Open' });
    } else {
      errors.push({ row: index + 1, message: parsed.error.issues.map((i) => i.message).join(' ') });
    }
  });

  const imported = dealsDb.bulkCreate(valid, user.name);
