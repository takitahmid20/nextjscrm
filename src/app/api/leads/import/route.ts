...
import { z } from 'zod';
import { leadImportRowSchema } from '@/src/validation';
...
export async function POST(request: NextRequest) {
  ...
  const valid: z.infer<typeof leadImportRowSchema>[] = [];
  const errors: { row: number; message: string }[] = [];

  parsedBody.data.rows.forEach((row, index) => {
    const parsed = leadImportRowSchema.safeParse(row);
    if (parsed.success) {
  ...
  const imported = leadsDb.bulkCreate(valid, user.name);
